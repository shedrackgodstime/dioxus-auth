...
fn find_credential_returns_subject_and_stored_hash() {
    ...
    let found = store.find_credential("bob@example.com").unwrap().unwrap();
...
fn find_credential_unknown_is_none() {
    ...
    let found = store.find_credential("ghost@example.com").unwrap();
...
fn rotate_secret_replaces_the_stored_hash() {
    ...
    let created = store.find_credential("bob").unwrap().unwrap();

    let new_hash = hash_password("new-secret");
    store.rotate_secret(&created.0.auth_id, &new_hash).unwrap();

    let found = store.find_credential("bob").unwrap().unwrap();
...
fn password_hash_is_not_material_for_lookup() {
    ...
    let found = store.find_credential("carol").unwrap().unwrap();
...
fn provision_rejects_a_taken_identifier_without_state_change() {
    ...
    let winner = store
        .provision_subject(None, TestUser::new(3, "bob"), "bob", &winner_hash)
        .unwrap()
        .expect("first claim must succeed");

    let taken = store
        .provision_subject(None, TestUser::new(4, "mallory"), "bob", &loser_hash)
        .unwrap();
    assert!(
        taken.is_none(),
...
fn provision_rejects_a_duplicate_app_id_without_state_change() {
    ...
    let taken = store
        .provision_subject(None, TestUser::new(3, "cloned-row"), "cloned-row", &hash)
        .unwrap();
    assert!(
        taken.is_none(),
    ...
    assert!(
        store.find_credential("cloned-row").unwrap().is_none(),
        "the losing credential row must not be written"
    );
    let survivor = store.find_credential("bob").unwrap().unwrap();
...
fn attach_adds_a_second_identifier_to_an_existing_subject() {
    ...
    let second_hash = hash_password("second");
    let created = store
        .provision_subject(None, TestUser::new(3, "bob"), "bob", &first_hash)
        .unwrap()
        .expect("claim must succeed");

    let attached = store
        .attach_credential(&created.auth_id, "bobby", &second_hash)
        .unwrap();
    assert!(attached);

    let added = store.find_credential("bobby").unwrap().unwrap();
    assert_eq!(added.0.auth_id, created.auth_id);
    assert_eq!(added.1, second_hash);
    let original = store.find_credential("bob").unwrap().unwrap();
...
fn attach_rejects_a_taken_identifier_without_state_change() {
    ...
    let attached = store
        .attach_credential(&created.auth_id, "bob", &hash_password("loser"))
        .unwrap();
...
fn attach_rejects_an_unknown_subject() {
    ...
    let result = store.attach_credential(&404, "ghost", &hash_password("pw"));
    assert_eq!(
        result.unwrap_err(),
        AuthError::InvalidCredentials,
        "attaching to a missing subject must fail without writing"
    );
    assert!(store.find_credential("ghost").unwrap().is_none());
}
...
fn rotate_secret_rewrites_all_rows_for_the_subject() {
    ...
    let created = store.find_credential("bob@example.com").unwrap().unwrap();
    store
