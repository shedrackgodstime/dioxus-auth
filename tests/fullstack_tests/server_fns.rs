...
use dioxus_auth::prelude::{
    ...
    SameSite, ServerAuthConfig, ServerAuthContext, ServerError, ServerFnError, SessionId,
    UserStore, current_user, write_session_cookie,
};
...
impl ProbingUserStore {
    ...
    fn record_lookup_thread(&self) {
        let mut slot = self
            .lookup_thread
            .lock()
            .expect("lookup thread slot must not be poisoned");
