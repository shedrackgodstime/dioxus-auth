...
- [ ] **<10 minutes** wall-clock: follow the README on a fresh checkout with
      a timer running. If it takes longer, the budget — not the reviewer —
...
- [ ] `graduation_preserves_verb_behavior` (root suite) and
      ...
      suite) both green — same verbs, same error codes across the boundary.
...
- [ ] No new required trait methods, no new required struct fields on
      `DefaultUser`, no verb signature changes — or a semver-major decision
